...
## 4. The Windows Hardening Execution
...
**Windows**
```powershell
.\0-environment_intake.ps1; .\1-baseline_snapshot.ps1; .\4-windows_harden.ps1
```
...
**Windows**
```powershell
$i = Get-Content .\environment_intake.json | ConvertFrom-Json; $b = Get-Content .\capstone\baseline\baseline_windows.json | ConvertFrom-Json; $w = Get-Content .\capstone\exec\windows_harden.json | ConvertFrom-Json; ($i.schema_version -and $i.captured_at -and $i.host.hostname) -and ($b.hostname -and ($null -ne $b.pass_rate_percent)) -and ($w.timestamp -and ($w.steps.Count -eq 7) -and ($null -ne $w.lynis_before) -and ($null -ne $w.lynis_after))
```